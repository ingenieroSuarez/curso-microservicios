...
export class FlightService {
    ...
    async addPassenger(flightId: string, passengerId: string): Promise<IFlight> {
        return await this.model.findByIdAndUpdate(flightId,
            {
                $addToSet: { passengers: passengerId },  // si el ID ya existe no lo vuelve a agregar, lo sustituye
            },
            {
                new: true
            }
        ).populate('passengers') //este plugin: cuando actualice retorna la información complet de los pasajeros
    }
